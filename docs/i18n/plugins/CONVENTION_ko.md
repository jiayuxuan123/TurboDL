...
이 문서는 TurboDL 프로젝트와 플러그인 작성자 사이의 공식 계약입니다. 이것은 호환성을 깨는 core 업데이트가 모든 플러그인을 조용히 망가뜨리는 일 없이 생태계가 성장할 수 있도록 존재합니다. 호환성 규칙이 생태계 전반에서 일관되게 유지되도록, TurboDL 프로젝트 자체가 유지관리합니다(서드파티에 위임하지 않음).

플러그인을 *작성*하기만 원한다면 [플러그인 제작 가이드](README_ko.md)에서 시작하세요; 이 문서는 그 가이드가 의존하는 *규칙집*입니다.
